...
def create_profile_transform(
    process_option: dict, data: Tensor | None = None
) -> tuple[Compose | None, tuple | None]:
    """Build transform pipeline from process_option dict.

    Args:
        process_option: dict with keys normalize, normalize_method,
            vectorize, style_vectorize, vectorize_window_size
        data: if provided, fit normalizer on this data

    Returns:
        (transform_pipeline, scaling_factor) or (None, None)
    """
    transforms = []
    scaling_factor = None

    if process_option.get("normalize"):
        method = process_option["normalize_method"]
        if method == "minmax":
            scaler = MinMaxScaler()
            if data is not None:
                scaler.fit(data)
                scaling_factor = (scaler.max_val, scaler.min_val)
            transforms.append(scaler)
        elif method == "meanstd":
            scaler = MeanStdScaler()
            if data is not None:
                scaler.fit(data)
                scaling_factor = (scaler.mean_val, scaler.std_val)
            transforms.append(scaler)
        else:
            raise ValueError(f"Invalid normalize method: {method}")

    if process_option.get("vectorize"):
        style = process_option["style_vectorize"]
        window = process_option["vectorize_window_size"]
        if style == "patchify":
            transforms.append(Patchify(window))
        elif style in ["chronological", "chrono"]:
            transforms.append(ChronoVectorize(window))
        elif style == "stft":
    ...
    if transforms:
        return Compose(transforms), scaling_factor
    return None, None
