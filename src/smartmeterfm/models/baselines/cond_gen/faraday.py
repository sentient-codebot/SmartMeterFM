...
class GaussianMixtureModel(nn.Module):
    ...
    def initialise(self, X: Tensor):
        ...
        precision_matrices = torch.linalg.inv(
            self.covariances
            + torch.eye(self.num_features, device=X.device) * self.reg_covar
        )
    ...
    def e_step(self, X: Tensor) -> tuple[Tensor, Tensor]:
        ...
        weighted_log_prob = log_prob + log_weights.unsqueeze(
            0
        )  # [n_samples, n_components]

        # Log-sum-exp for numerical stability
        log_prob_norm = torch.logsumexp(weighted_log_prob, dim=1)  # [n_samples]
        log_resp = weighted_log_prob - log_prob_norm.unsqueeze(
            1
        )  # [n_samples, n_components]
    ...
    def m_step(
        self, X: Tensor, log_responsibilities: Tensor
        ...
        for k in range(self.num_components):
            try:
                ...
                cov_reg = (
                    covariances[k]
                    + torch.eye(self.num_features, device=X.device) * 1e-4
                )
